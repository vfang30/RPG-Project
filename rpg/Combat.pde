...
class Combat{
  ...
  void keyPressedCombat(){
    if (!enemyTurn() && !attack){
      ...
      if (keyCode == 'X'){
        ...
        if (option == 0 && menu && !attack && !dmgStep){
        optionReset();
        menu = false;
        moves = true;

        }else if (option == 1 && menu && !attack && !dmgStep) {
        optionReset();
        current.mana += (int)(current.maxMana/2);
        ...
        }else if (option == 2 && menu && !attack && !dmgStep) {
        optionReset();
        menu = false;
        item = true;
  ...
  void run(){
   ...
   void enemyAttack(){
     ...
     selected = party.get(target);
     atkMsg = current.attack(selected);
     println(atkMsg);
   }
